...
Shared utilities for all Streamlit pages:
  - async_run() — run coroutines from sync Streamlit context
  - apply_theme() — dark institutional styling
  - chart utilities
"""
...
<style>
  ...
  /* Score badge colours */
  .score-high  { color: #00d2a8; font-weight: 700; }
  .score-med   { color: #f5c518; font-weight: 700; }
  .score-low   { color: #ff4b6e; font-weight: 700; }
...
def async_run(coro: Any) -> Any:
    ...
    Handles both the case where an event loop is already running and when it's not.
    """
    try:
...
def candlestick_chart(
    ...
    fig.add_trace(
        go.Candlestick(
            ...
            close=df["close"],
            name="Price",
            increasing_line_color=BULL_GREEN,
            decreasing_line_color=BEAR_RED,
            increasing_fillcolor=BULL_GREEN,
            decreasing_fillcolor=BEAR_RED,
        ),
        row=1, col=1,
    )
    ...
    if "volume" in df.columns:
        colors = [
            BULL_GREEN if df["close"].iloc[i] >= df["open"].iloc[i] else BEAR_RED
...
def cot_index_chart(df: pd.DataFrame, symbol: str) -> go.Figure:
    fig = go.Figure()
...
def regime_timeline(dates: list, values: list, title: str, threshold: float = 0) -> go.Figure:
...
def seasonality_heatmap(symbol: str) -> go.Figure:
...
def render_signal_card(sig: Any) -> None:
    """Render a signal as an HTML card."""
    ...
    <div class="signal-card {dir_class}">
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:8px; font-size:0.85rem; color:{TEXT_DIM};">
        ...
        <div>⚖️ R:R: <b style="color:{TEXT_PRIMARY};">{sig.risk_reward:.1f}x</b></div>
