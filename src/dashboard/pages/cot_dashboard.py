...
for i, (sym, name) in enumerate(ALL_MARKETS.items()):
    ...
    with col:
        if idx_val is not None:
            ...
            signal = "BULLISH" if idx_val >= 70 else "BEARISH" if idx_val <= 30 else "NEUTRAL"
            st.markdown(
...
selected = st.selectbox("Select Market", options=list(ALL_MARKETS.keys()),
                         format_func=lambda x: f"{x} - {ALL_MARKETS[x]}")
...
tab1, tab2, tab3 = st.tabs(["COT Index", "Positions", "Weekly Changes"])
...
with tab1:
    ...
    if "cot_index" in df_sel.columns:
        ...
        c4.metric("52-Week Range", f"{s.tail(52).min():.0f}-{s.tail(52).max():.0f}")
...
with tab2:
    if "comm_long" in df_sel.columns and "comm_short" in df_sel.columns:
        ...
        fig2.add_trace(go.Bar(x=df_pos.index, y=df_pos["comm_long"], name="Comm Long",
                              marker_color=BULL_GREEN, opacity=0.7), row=1, col=1)
        fig2.add_trace(go.Bar(x=df_pos.index, y=df_pos["comm_short"], name="Comm Short",
                              marker_color=BEAR_RED, opacity=0.7), row=1, col=1)
        colors_net = [BULL_GREEN if v >= 0 else BEAR_RED for v in df_pos["comm_net"]]
        fig2.add_trace(go.Bar(x=df_pos.index, y=df_pos["comm_net"], name="Net",
                              marker_color=colors_net), row=2, col=1)
...
with tab3:
    if "comm_long_chg" in df_sel.columns:
        ...
        fig3 = go.Figure()
        fig3.add_trace(go.Bar(x=df_chg.index, y=df_chg["comm_long_chg"], name="Long Change",
                              marker_color=BULL_GREEN, opacity=0.8))
        fig3.add_trace(go.Bar(x=df_chg.index, y=df_chg["comm_short_chg"], name="Short Change",
                              marker_color=BEAR_RED, opacity=0.8))
        fig3.update_layout(**{**PLOTLY_LAYOUT, "height": 350, "barmode": "group",
                               "title": f"{ALL_MARKETS[selected]} - Weekly COT Position Changes"})
        st.plotly_chart(fig3, use_container_width=True)
    else:
        st.info("Weekly change data unavailable.")

st.divider()
st.caption("COT data source: CFTC Disaggregated Futures-Only Report - Released every Friday 15:30 ET")
