...
c1.metric("VIX", f"{vix:.2f}", delta="OVERRIDE" if vix > 35 else None, delta_color="inverse")

dxy_r = result.regime.dxy_regime.title()
c2.metric("DXY Regime", dxy_r, delta="Strong $" if result.regime.dxy_bullish else "Weak $")

us10y_r = "Above MA200" if result.regime.us10y_above_ma else "Below MA200"
c3.metric("US10Y Regime", us10y_r, delta="Equity Headwind" if result.regime.us10y_above_ma else "Equity Tailwind")

ry_r = result.regime.real_yield_regime.title()
c4.metric("Real Yield", ry_r, delta="Avoid Gold/Silver Longs" if result.regime.real_yield_rising else "Gold/Silver Favourable")

cash_req = "30%" if result.aggregate_macro_score < 48 else "15%"
c5.metric("Cash Reserve Required", cash_req, delta=f"Macro Score: {result.aggregate_macro_score:.0f}/100")
...
with col_right:
    ...
    if result.signals:
        ...
        fig.add_hline(y=52, line_dash="dash", line_color=GOLD, annotation_text="Threshold 52")
        fig.update_layout(**{
