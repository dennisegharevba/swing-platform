...
if above_ma:
    st.warning(f"US10Y {yield_now:.3f}% - Above MA200 ({ma200:.3f}%) - Headwind for equity longs / Favour equity shorts")
else:
    st.success(f"US10Y {yield_now:.3f}% - Below MA200 ({ma200:.3f}%) - Allow equity longs / Avoid equity shorts")
...
if "ma_200" in df.columns:
    ...
    fig_d = go.Figure(go.Bar(x=diff_series.index, y=diff_series.values, marker_color=colors_d, name="Yield - MA200"))
    fig_d.add_hline(y=0, line_color=TEXT_DIM, line_width=1)
    fig_d.update_layout(**{**PLOTLY_LAYOUT, "height": 260, "title": "10Y Yield minus MA200"})
...
ry_label = "Rising - Avoid Gold/Silver Longs" if ry_chg > 0 else "Falling - Gold/Silver Longs Allowed"
if ry_chg > 0:
...
|----------------|-------------|---------------|
| Below MA200 | Allowed - full macro score | Reduced macro score |
| Above MA200 | Reduced macro score | Allowed - full macro score |
