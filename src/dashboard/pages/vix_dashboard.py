...
if vix_now > 35:
    ...
    st.warning(f"VIX Elevated: {vix_now:.2f} - Reduce position sizing")
else:
...
fig.add_trace(go.Scatter(
    x=vix_series.index, y=vix_series.values, name="VIX",
    line=dict(color="#e0e0e0", width=2), fill="tozeroy", fillcolor="rgba(200,200,200,0.05)",
))

for y, color, label in [(35, BEAR_RED, "Override Threshold (35)"), (25, "#ff8c00", "High Zone (25)"), (20, GOLD, "Elevated Zone (20)")]:
    fig.add_hline(y=y, line_dash="dash", line_color=color, annotation_text=label, annotation_position="right")

fig.update_layout(**{**PLOTLY_LAYOUT, "height": 380, "title": "VIX - CBOE Volatility Index (252 days)"})
...
fig_z = go.Figure(go.Bar(
    x=list(zones.keys()), y=list(zones.values()), marker_color=colors_z,
    text=[f"{v} days ({v/252*100:.0f}%)" for v in zones.values()], textposition="outside",
))
